---
layout: default
title: 股票财经
---

<h1>📈 股票与宏观财经看板</h1>
<p class="page-subtitle">全球主要股指追踪 · A股/港股/美股核心资产 · 主力板块资金流向 · 每日宏观财经资讯聚合与深度研报</p>

<div class="news-meta-bar">
  <span class="news-meta-item">📊 全球市场指数</span>
  <span class="news-meta-item">🔥 主力资金流向</span>
  <span class="news-meta-item">⚡ 核心赛道透视</span>
  <span class="news-meta-item">📰 每日财经资讯</span>
  <span class="news-meta-item">💡 悬浮即览深度简述</span>
  <span class="news-meta-item">🕐 每日自动更新</span>
</div>

<!-- ================= 1. 全球核心指数行情看板 ================= -->
<div class="section-header" style="margin-top: 24px;">
  <h2 style="font-size: 18px; margin: 0; display: flex; align-items: center; gap: 8px;">
    <span>🌍 全球核心股指 & 宏观资产快照</span>
  </h2>
  <span style="font-size: 12px; color: var(--color-muted);">基准行情参考 · 日级走势 · 9/9 项实时行情</span>
</div>

<div class="finance-ticker-grid">
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🇨🇳 上证指数</span>
      <span class="ticker-code">000001.SH</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-up">3,949.91</span>
      <span class="ticker-change up">▲ +0.97%</span>
    </div>
    <div class="ticker-footer">
      <span>震荡筑底中枢</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🇨🇳 深证成指</span>
      <span class="ticker-code">399001.SZ</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-up">13,730.02</span>
      <span class="ticker-change up">▲ +0.65%</span>
    </div>
    <div class="ticker-footer">
      <span>成长与制造共振</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🇨🇳 创业板指</span>
      <span class="ticker-code">399006.SZ</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-up">3,399.59</span>
      <span class="ticker-change up">▲ +0.80%</span>
    </div>
    <div class="ticker-footer">
      <span>新能源 & 医药领跑</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🇨🇳 科创50</span>
      <span class="ticker-code">000688.SH</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-up">1,657.48</span>
      <span class="ticker-change up">▲ +0.29%</span>
    </div>
    <div class="ticker-footer">
      <span>AI算力与先进制程</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🇭🇰 恒生科技</span>
      <span class="ticker-code">HSTECH</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-up">4,423.29</span>
      <span class="ticker-change up">▲ +0.40%</span>
    </div>
    <div class="ticker-footer">
      <span>互联网平台回购加码</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🇺🇸 纳斯达克100</span>
      <span class="ticker-code">NDX</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-up">30,431.77</span>
      <span class="ticker-change up">▲ +0.01%</span>
    </div>
    <div class="ticker-footer">
      <span>科技巨头财报韧性</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">💱 美元/离岸人民币</span>
      <span class="ticker-code">USD/CNH</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-down">6.6926</span>
      <span class="ticker-change down">▼ -0 bp</span>
    </div>
    <div class="ticker-footer">
      <span>人民币汇率稳健调升</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🪙 伦敦现货黄金</span>
      <span class="ticker-code">XAU/USD</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-down">$4,351.17 <span style="font-size:12px;font-weight:normal;color:var(--color-muted);">/盎司</span></span>
      <span class="ticker-change down">▼ -27.12 (-0.62%)</span>
    </div>
    <div class="ticker-footer">
      <span>央行购金与全球避险</span>
      
    </div>
  </div>
  <div class="ticker-card">
    <div class="ticker-header">
      <span class="ticker-name">🪙 国内现货黄金</span>
      <span class="ticker-code">Au(T+D)</span>
    </div>
    <div class="ticker-body">
      <span class="ticker-price ticker-down">¥938.05 <span style="font-size:12px;font-weight:normal;color:var(--color-muted);">/克</span></span>
      <span class="ticker-change down">▼ -0.06 (-0.01%)</span>
    </div>
    <div class="ticker-footer">
      <span>上海黄金交易所基准</span>
      
    </div>
  </div>
</div>


<!-- ================= 2. 热门核心赛道与主力资金流向 ================= -->
<div class="section-header" style="margin-top: 32px;">
  <h2 style="font-size: 18px; margin: 0; display: flex; align-items: center; gap: 8px;">
    <span>🔥 核心热门主线赛道 & 资金流向透视</span>
  </h2>
  <span style="font-size: 12px; color: var(--color-muted);">主力净流入与产业催化（参考值）</span>
</div>

<div class="finance-sector-grid">
  <div class="sector-card">
    <div class="sector-title-row">
      <span class="sector-title">🤖 人工智能 & 先进算力链</span>
      <span class="sector-flow-badge">+21.6 亿</span>
    </div>
    <p class="sector-desc">基于近期产业政策与市场热点的资金流向参考，实际数据以交易所公布为准。</p>
    <div class="sector-tags">
      <span class="sector-tag-chip">CPO 光模块</span>
      <span class="sector-tag-chip">GPU 服务器</span>
      <span class="sector-tag-chip">先进制程封装</span>
      <span class="sector-tag-chip">PCB 算力板</span>
    </div>
  </div>
  <div class="sector-card">
    <div class="sector-title-row">
      <span class="sector-title">⚡ 新能源出海 & 特高压电网</span>
      <span class="sector-flow-badge">+42.6 亿</span>
    </div>
    <p class="sector-desc">基于近期产业政策与市场热点的资金流向参考，实际数据以交易所公布为准。</p>
    <div class="sector-tags">
      <span class="sector-tag-chip">特高压直流</span>
      <span class="sector-tag-chip">固态锂电</span>
      <span class="sector-tag-chip">工商业储能</span>
      <span class="sector-tag-chip">核电核岛</span>
    </div>
  </div>
  <div class="sector-card">
    <div class="sector-title-row">
      <span class="sector-title">🚗 具身智能 & 智能网联车</span>
      <span class="sector-flow-badge">+40.4 亿</span>
    </div>
    <p class="sector-desc">基于近期产业政策与市场热点的资金流向参考，实际数据以交易所公布为准。</p>
    <div class="sector-tags">
      <span class="sector-tag-chip">端到端智驾</span>
      <span class="sector-tag-chip">激光雷达</span>
      <span class="sector-tag-chip">行星滚柱丝杠</span>
      <span class="sector-tag-chip">六维力传感器</span>
    </div>
  </div>
  <div class="sector-card">
    <div class="sector-title-row">
      <span class="sector-title">🛡️ 高股息红利与底仓资产</span>
      <span class="sector-flow-badge">+34.2 亿</span>
    </div>
    <p class="sector-desc">基于近期产业政策与市场热点的资金流向参考，实际数据以交易所公布为准。</p>
    <div class="sector-tags">
      <span class="sector-tag-chip">国有大行</span>
      <span class="sector-tag-chip">水电公用</span>
      <span class="sector-tag-chip">高股息煤炭</span>
      <span class="sector-tag-chip">中字头基建</span>
    </div>
  </div>
</div>


<!-- ================= 3. 每日财经资讯 ================= -->
<div class="section-header" style="margin-top: 36px;">
  <h2 style="font-size: 18px; margin: 0; display: flex; align-items: center; gap: 8px;">
    <span>📰 每日宏观财经 & 资本市场资讯聚合</span>
  </h2>
  <span style="font-size: 12px; color: var(--color-muted);">鼠标悬停即可查看深度微型特稿与背景剖析</span>
</div>

<div class="news-grid">
  <div class="news-category">
    <div class="news-category-header">
      <span class="category-flag">💰</span>
      <span class="news-category-title">宏观经济 · 汇率 · 证券 · 产业深度</span>
      <span class="news-category-count">2 条精选资讯</span>
    </div>
        <a class="news-item" href="https://www.tomshardware.com/tech-industry/data-centers/local-opposition-blocked-usd68-billion-worth-of-data-center-projects-in-the-second-quarter-of-2026-data-center-investments-reportedly-still-on-track-to-hit-usd32-trillion-by-2050" target="_blank" rel="noopener" data-cat="caijing" data-summary="尽管投资仍在增加，但当地对数据中心建设的反对意见在2026年第二季度阻止了价值680亿美元的数据中心项目。" data-title="当地反对派在2026年第二季度阻止了45个价值680亿美元的数据中心项目—据报道，到2050年，数据中心投资仍有望达到32万亿美元" data-date="09-22 00:00" data-source="Tom's Hardware">
          <span class="news-cat-tag cat-caijing">💰 财经资讯</span>
          <span class="source-badge source-tomshardware">⚡ Tom's Hardware</span>
          <span class="news-item-date">09-22 00:00</span>
          <span class="news-item-title">当地反对派在2026年第二季度阻止了45个价值680亿美元的数据中心项目—据报道，到2050年，数据中心投资仍有望达到32万亿美元</span>
        </a>
        <a class="news-item" href="https://www.chinanews.com.cn/sh/2026/09-21/10701098.shtml" target="_blank" rel="noopener" data-cat="caijing" data-summary="中新网威海9月21日电 (记者 王娇妮)21日，“五链融合”发展大会暨2026中国威海·国际英才创新创业大会在山东威海开幕。来自高校院所、投融资机构、重点企业的千余名嘉宾齐聚一堂，共赴“链”上之约。" data-title="山东威海“五链融合”发展大会举办 千余名嘉宾共赴“链”上之约" data-date="09-21 22:13" data-source="中国新闻网">
          <span class="news-cat-tag cat-caijing">💰 财经资讯</span>
          <span class="source-badge source-cn">🇨🇳 中国新闻网</span>
          <span class="news-item-date">09-21 22:13</span>
          <span class="news-item-title">山东威海“五链融合”发展大会举办 千余名嘉宾共赴“链”上之约</span>
        </a>
  </div>
</div>


<div style="text-align: center; margin: 36px 0 20px;">
  <a href="{{ "/news" | relative_url }}" class="card-link" style="display: inline-flex; align-items: center; gap: 6px; padding: 10px 24px; font-size: 14px;">
    <span>📰 返回综合热点新闻专区</span>
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"></polyline></svg>
  </a>
</div>

---

<p class="news-updated">🕐 数据抓取于 2026-09-22 02:14（北京时间）· 股指数据来源新浪财经 · 资金流向为参考估算 · 仅供参考不构成任何投资建议</p>
